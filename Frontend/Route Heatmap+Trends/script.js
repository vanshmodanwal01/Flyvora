document.addEventListener("DOMContentLoaded", () => {
    ...
    const kpi30DayChange = document.querySelector("#kpi-30day-change .metric-value");
    const kpiIndexWeight = document.querySelector("#kpi-index-weight .metric-value");
    const kpiObservations = document.querySelector("#kpi-observations .metric-value");

    // Breakdown Elements
    const breakdownHeader = document.querySelector("#airline-breakdown-header h3");
    const tileIndigo = document.querySelector("#airline-tile-indigo .metric-value");
    const tileAirIndia = document.querySelector("#airline-tile-air-india .metric-value");
    const tileSpiceJet = document.querySelector("#airline-tile-spicejet .metric-value");
        ...
        options: {
            ...
            plugins: {
                legend: {
                    ...
                    position: "top",
                    align: "end",
                    labels: { color: "#94a3b8", font: { size: 11, family: "sans-serif" }, usePointStyle: true, boxWidth: 6 }
                },
                tooltip: {
                    mode: "index",
                    intersect: false,
    ...
    function updateDashboard(routeKey) {
        ...
        if (data.changeTrend === "up") {
            kpi30DayChange.className = "metric-value text-3xl font-bold text-red-400 mt-2";
            kpi30DayChange.innerHTML = `${data.change30D} <span class="text-sm inline-block transform translate-y-[-1px]">▲</span>`;
        } else {
            kpi30DayChange.className = "metric-value text-3xl font-bold text-emerald-400 mt-2";
            kpi30DayChange.innerHTML = `${data.change30D} <span class="text-sm inline-block transform translate-y-[1px]">▼</span>`;
        }
        ...
        triggerPopAnimation([
            kpiAvgFare, kpi30DayChange, kpiIndexWeight, kpiObservations,
            tileIndigo, tileAirIndia, tileSpiceJet, tileAkasa
        ]);

        // Update Chart Title & Data Smoothly
        const chartTitle = document.querySelector("#price-trend-card h2");
        if (chartTitle) chartTitle.textContent = `Price Trend — ${data.name} vs National Avg`;
