...
const { days: leadTimeDays, prices: leadTimePrices } =
  buildElasticityDataset(4120);
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label: (ctx) =>
              `Avg Fare: ₹${ctx.parsed.y.toLocaleString("en-IN")}`,
          },
        },
      },
      scales: {
...
const routeComparisonCtx = document.getElementById(
...
if (routeComparisonCtx) {
  ...
  const datasets = ROUTE_COMPARISON.map((route) => {
    const data = sampleDays.map((d) => {
      ...
      const gentle = daysOut * 6;
      const steep =
        daysOut > 30 ? Math.pow(daysOut - 30, 2.15) * route.steepness : 0;
